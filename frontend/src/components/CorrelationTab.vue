...
function disposeCharts() {
  ...
  if (cumulativeChart) { cumulativeChart.dispose(); cumulativeChart = null }
}

onUnmounted(() => disposeCharts())

function renderCharts() {
  const returns: ReturnPoint[] = props.result?.returns
  if (!returns?.length) return

  const stockRet = returns.map((r) => r.stock_return)
  const benchRet = returns.map((r) => r.benchmark_return)
  ...
  const beta = props.result.beta ?? 0
  const alpha = props.result.alpha ?? 0
  ...
  scatterChart.setOption({
    tooltip: {
      trigger: 'item',
      formatter: (p: any) =>
        `个股: ${(p.value[1] * 100).toFixed(2)}%<br/>基准: ${(p.value[0] * 100).toFixed(2)}%`,
    },
    xAxis: {
      name: '基准收益率',
  ...
  for (const r of returns) {
    ...
    dates.push(r.trade_date.slice(0, 10))
  ...
  cumulativeChart.setOption({
    ...
    legend: { data: [props.stockName, props.benchmarkName], bottom: 0 },
    ...
    <el-row :gutter="16" style="margin-bottom: 16px">
      ...
      <el-col :span="4">
        <el-card shadow="hover">
          ...
          <div style="font-size: 11px; color: #909399">p = {{ result.f_pvalue?.toFixed(6) ?? 'N/A' }}</div>
        </el-card>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="16" style="margin-bottom: 20px">
      <el-col :span="12">
        <el-card shadow="hover">
          ...
          <el-descriptions :column="3" size="small" border>
            <el-descriptions-item label="p 值">{{ result.beta_pvalue?.toFixed(6) ?? 'N/A' }}</el-descriptions-item>
      ...
      <el-col :span="12">
        <el-card shadow="hover">
          ...
          <el-descriptions :column="3" size="small" border>
            <el-descriptions-item label="p 值">{{ result.alpha_pvalue?.toFixed(6) ?? 'N/A' }}</el-descriptions-item>
